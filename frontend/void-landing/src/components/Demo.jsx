...
import { Send, Bot, Video, Keyboard, Loader2, Mic, MicOff, Circle, Square, ChevronDown, List, Activity, Heart, AlertTriangle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

// 1. تعريف المشاعر والألوان (The Sentiment Logic)
...
const PREDEFINED_SENTENCES = [
  { id: 'hello', ar: "أهلاً / السلام عليكم", en: "Hello", mood: 'neutral' },
  { id: 'all_good', ar: "كله تمام / الحمد لله", en: "All Good", mood: 'positive' },
  { id: 'what_is_your_name', ar: "اسمك ايه؟", en: "What is your name?", mood: 'neutral' },
  { id: 'hospital', ar: "أنا محتاج مستشفى", en: "I need a hospital", mood: 'danger' },
  { id: 'help', ar: "ساعدني / نجدة", en: "Help me", mood: 'danger' },
  { id: 'hungry', ar: "أنا جعان / عايز آكل", en: "I am hungry", mood: 'neutral' },
  { id: 'tired', ar: "أنا تعبان / مش كويس", en: "I am tired", mood: 'danger' },
  { id: 'love', ar: "أنا بحبك", en: "I love you", mood: 'positive' },
  { id: 'quran', ar: "أنا بحفظ قرآن", en: "I memorize Quran", mood: 'positive' },
];
...
export default function Demo() {
  ...
  const [currentMood, setCurrentMood] = useState('neutral'); // neutral, danger, positive
  ...
  useEffect(() => {
    ...
    else if (text.match(/love|good|happy|thanks|great|أحبك|تمام|شكرا|سعيد|ممتاز|الحمد لله/i)) {
  ...
  const sendVideoToBackend = async (videoBlob) => {
    ...
    try {
      ...
      console.error(error);
      setOutputText(lang === 'ar' ? "خطأ في الاتصال" : "Connection Error");
    } finally {
  ...
  const startRecording = useCallback(() => {
    ...
    if (webcamRef.current && webcamRef.current.video.srcObject) {
      const mimeType = MediaRecorder.isTypeSupported("video/webm; codecs=vp8") 
                 ...
                 : "video/webm"; // لو مش مدعوم ارجع للعادي

      // 2. نبدأ التسجيل بالصيغة المضمونة
  ...
  return (
    <section id="demo" className="py-24 bg-slate-50 relative overflow-hidden transition-colors duration-700">
      ...
      {/* 🚨 Feature 1: Sound Awareness Visualizer (Header) */}
      <div className="absolute top-4 right-4 md:top-10 md:right-10 flex items-center gap-2 z-20">
         <div className="bg-white/80 backdrop-blur-md px-4 py-2 rounded-full shadow-sm flex items-center gap-3 border border-slate-200">
            ...
            <div className="flex items-end gap-1 h-6">
                ...
                <motion.div animate={{ height: [12, 22, 12] }} transition={{ repeat: Infinity, duration: 0.3 }} className={`w-1 rounded-full ${currentMood === 'danger' ? 'bg-red-500' : 'bg-slate-400'}`}></motion.div>
            </div>
                ...
                <motion.div key="text-input" initial={{opacity:0, x:-20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:20}} className="flex-1 flex flex-col gap-4">
                  ...
                  <div className="relative group">
                    ...
                    <select
                        ...
                        dir={lang === 'ar' ? 'rtl' : 'ltr'}
                    >
                        <option value="">{lang === 'ar' ? '-- جمل جاهزة --' : '-- Quick Select --'}</option>
                  ...
                  <div className={`flex-1 bg-white rounded-2xl p-2 shadow-inner border focus-within:ring-2 transition-all relative flex flex-col ${theme.border} ${theme.ring}`}>
                    ...
                    <div className="flex justify-between items-center p-2 border-t border-slate-100">
                        ...
                        <button 
                            ...
                            className={`p-3 rounded-full transition-all ${isListening ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-slate-100 text-slate-500 hover:text-teal-600'}`}
                        >
